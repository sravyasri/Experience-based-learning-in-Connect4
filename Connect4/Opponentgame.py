...
MovesofPlayer=0
EndResult = open('ENDRESULT', 'a+')
EndResult.write("NEXT SET"+"\n")

for i in range(0,1000):
    print "Opponent Game"
    try:
        game = connectfour.games.standard.StandardGame(p1=players.negamax.NegamaxPlayer(4), p2=players.negamax.NegamaxPlayer(2), verbose=True)
        game.play()
    ...
    if game.getWinner() == 2:
            ...
            Draws = Draws + 1
EndResult.write("Player1WinsOpponent="+str(Player1Win)+"\n")
EndResult.write("Player2WinsOpponent="+str(Player2Win)+"\n")
